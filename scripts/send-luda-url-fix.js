...
const BODY_HTML = `<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#222222;line-height:1.55;max-width:680px">
  <p>Hi Luda,</p>
  <p>Quick follow-up on my note about the screenshots. I just deployed a fix so the <strong>/analysis URLs in the walkthrough now work correctly</strong>. Clicking any of them lands on the proper review page where the change is visible directly.</p>
  <p>For the 7 items where the screenshot does not have a red highlight (03, 04, 05, 06, 08, 13, 17), the URL on the card is now the cleanest verification path. Click the URL, the page loads, and you can mark it Approved / Issue / Question against what is described on the card.</p>
  <p>You should not need to mark any of those 7 purely because the screenshot lacks a highlight box.</p>
  <p>Best,</p>
</div>${SIGNATURE_HTML}`;

const BODY_TEXT = `Hi Luda,

Quick follow-up on my note about the screenshots. I just deployed a fix so the /analysis URLs in the walkthrough now work correctly. Clicking any of them lands on the proper review page where the change is visible directly.

For the 7 items where the screenshot does not have a red highlight (03, 04, 05, 06, 08, 13, 17), the URL on the card is now the cleanest verification path. Click the URL, the page loads, and you can mark it Approved / Issue / Question against what is described on the card.

You should not need to mark any of those 7 purely because the screenshot lacks a highlight box.
...
async function send() {
  if (!SHOULD_SEND) {
    ...
    console.log('---');
    console.log('To send for real: node scripts/send-luda-url-fix.js --send');
    return;
  }
